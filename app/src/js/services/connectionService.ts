// Connection service to manage all connection functionality
import { ConnectionInstance, ConnectionMode, ConnectionState, NodeInstance } from '../models/types.js';
 ...
 */
function setupInputPort(portElement: HTMLElement, nodeElement: HTMLElement, nodeInstance: NodeInstance): void {
 ...
 */
function setupOutputPort(portElement: HTMLElement, nodeElement: HTMLElement, nodeInstance: NodeInstance): void {
    portElement.addEventListener('click', event => {
        event.stopPropagation(); // Prevent node selection
        ...
        const portCategory = portElement.dataset.portCategory ||
        ...
        // Create temporary line that follows the mouse pointer
        const tempLine = new LeaderLine(
            portElement,
            { x: event.pageX, y: event.pageY }, // Initial mouse position
            {
                path: 'fluid',
                startPlug: 'disc',
                endPlug: 'arrow3',
        ...
        if (canvas) {
            ...
            const mouseMoveHandler = (e: MouseEvent) => {
                if (connectionState.mode === ConnectionMode.CONNECTING && connectionState.tempLine) {
                    // Update end position of the line
                    connectionState.tempLine.position();
                    connectionState.tempLine.end = { x: e.pageX, y: e.pageY };
                }
            };
 ...
 */
export async function createConnection(fromNodeId: string, fromPortId: string, toNodeId: string, toPortId: string): Promise<ConnectionInstance | null> {
    try {
        ...
        const portCategory = fromPortElement.dataset.portCategory ||
            (fromPortElement.dataset.portType === PortType.CONTROL ? PortCategory.FLOW : PortCategory.DATA);
 ...
 */
function checkPortsCompatibility(sourcePort: HTMLElement, targetPort: HTMLElement): { compatible: boolean, reason: string } {
    ...
    // Get port categories
    const sourceCategory = sourcePort.dataset.portCategory ||
        (sourcePort.dataset.portType === PortType.CONTROL ? PortCategory.FLOW : PortCategory.DATA);

    const targetCategory = targetPort.dataset.portCategory ||
